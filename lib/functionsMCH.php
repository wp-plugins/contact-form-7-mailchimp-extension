...
function wpcf7_cm_subscribe($obj)
{
	...
	if( $cf7_cm )
	{
		...
		if($subscribe && $email != $cf7_cm['email'])
		{
			...
			$wrap = new Mailchimp($cf7_cm['api']);
			$Mailchimp = new Mailchimp( $cf7_cm['api'] );
			...
			foreach($listarr as $listid)
			{
                             /* $merge_vars = array('FNAME'=>$name, 'LNAME'=>'Gilmour',
								...
								//echo 'llego aqui';
                                $merge_vars = array('FNAME'=>$name, 'LNAME'=>'xxxx');
                                $listid = trim($listarr[0]);
                                /*
                                echo 'Fname:'.$name.' ';
                                echo 'Email:'.$email.' ';
                                echo 'list:'.$listid.' ';
                                echo 'ApiKey:'.$cf7_cm['api'].'  ';*/
                                ...
                                /*//este funciona
								$subscriber = $Mailchimp_Lists->subscribe($listid,
								array( 'email' => $email ));
								*/
